function [voices, nmat8, probRecords] = marker2marker(nmat8, ...
                lastMarker, noOfVoices, endPoint, HMMdata, probRecords)
% [voices, nmat8, probRecords] = marker2marker(nmat8, ...
%                lastMarker, noOfVoices, endPoint, HMMdata, probRecords)
%
...
%     % Find other voices already allocated at that beat
%     %allocationPoints = findOtherAllocations(nmat8, currentBeat);
%     noOfAllocations = size(allocationPoints, 1);
%     if noOfAllocations > 0
%         for i=1:noOfAllocations
%             if allocationPoints(i, 2) = prob(1, 1)
%                 % then we have two notes being allocated to the same voice
%                 if allocationPoints(i, 3) >= prob(1, 2)
%                     % keep previous allocation as it is and allocate the
%                     % next note to the next highest prob
%                   % ***************** %
